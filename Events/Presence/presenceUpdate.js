...
const filePath = path.join(__dirname, "../../Database/Üye Verileri/sonGorulme.json");
const dataPath = path.join(__dirname, "../../Database/Sunucu Yönetimi/durumRol.json");
    ...
    client.on("presenceUpdate", async (oldPresence, newPresence) => {
        ...
        if (currentState) {
            if (!member.roles.cache.has(guildData.rolId)) {
                ...
                if (logChannel) {
                    await logChannel.send({
                        content: `${emojiler.tik} <@${user.id}> **(${user.id})** durumuna **${guildData.tag}** tagını eklediği için **rolü verildi.**`,
                        components: [rowAdd]
                    });
                }
            }
            ...
            if (member.roles.cache.has(guildData.rolId)) {
                ...
                if (logChannel) {
                    await logChannel.send({
                        content: `${emojiler.carpi} <@${user.id}> **(${user.id})** durumundan **${guildData.tag}** tagını sildiği için **rolü alındı.**`,

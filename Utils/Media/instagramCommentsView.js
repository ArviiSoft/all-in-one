...
function pageCustomId(instagramId, ownerId, page, role) {
  return `instagramshowcomments_${instagramId}:${ownerId}:${page}:${role}`;
}
...
function navigationRow(instagramId, ownerId, page, totalPages) {
  return new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId(pageCustomId(instagramId, ownerId, Math.max(0, page - 1), 'prev'))
      .setEmoji('⬅️')
      .setStyle(ButtonStyle.Secondary)
      .setDisabled(page === 0),
    new ButtonBuilder()
      .setCustomId(pageCustomId(instagramId, ownerId, page, 'current'))
      .setLabel(`${page + 1}/${totalPages}`)
      .setStyle(ButtonStyle.Secondary)
      .setDisabled(true),
    new ButtonBuilder()
      .setCustomId(pageCustomId(instagramId, ownerId, Math.min(totalPages - 1, page + 1), 'next'))
      .setEmoji('➡️')
      .setStyle(ButtonStyle.Secondary)
...
function buildInstagramCommentsView({ instagram, instagramId, ownerId, page = 0 }) {
  const comments = Array.isArray(instagram?.yorumlar) ? instagram.yorumlar : [];
  ...
  const handle = safeText(instagram?.username, 'kullanici', 80);

  const container = new ContainerBuilder()
    .setAccentColor(INSTAGRAM_PINK)
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent([
        '## 💬 Instagram Yorumları',
        `-# @${handle} gönderisine yapılan yorumlar • ${comments.length} yorum`,
      ].join('\n'))
    )
  ...
  if (!pageComments.length) {
    container.addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        '### Henüz yorum yok\nBu gönderiye ilk yorumu sen yapabilirsin.'
      )
    );
  } else {
  ...
  container
    .addSeparatorComponents(separator())
    .addActionRowComponents(navigationRow(instagramId, ownerId, safePage, totalPages));
